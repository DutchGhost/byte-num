...
pub mod bench_from_ascii {
    ...
    #[bench]
    fn bench_u64_from_ascii_FA(b: &mut Bencher) {
        b.iter(|| {
            assert_eq!(
                STRING_U64.iter().cloned().map(u64::atoi2).all(|n| n.is_ok()),
                true
            );
        })
    }
    ...
    fn bench_u16_std_parse(b: &mut Bencher) {
        b.iter(|| {
            assert_eq!(
                STRING_U16.iter().cloned().map(|s| s.parse::<u16>()).all(
                    |n| {
                        n.is_ok()
                    },
                ),
    ...
    fn bench_u32_std_parse(b: &mut Bencher) {
        b.iter(|| {
            assert_eq!(
                STRING_U32.iter().cloned().map(|s| s.parse::<u32>()).all(
                    |n| {
                        n.is_ok()
                    },
                ),
...
pub mod bench_into_ascii {
    ...
    pub mod benches {
        ...
        fn bench_u16_into_ascii(b: &mut Bencher) {
            b.iter(|| for item in INT_U16.iter() {
                item.itoa();
            })
        }
        ...
        fn bench_u64_into_ascii(b: &mut Bencher) {
            b.iter(|| for item in INT_U64.iter() {
                item.itoa();
            })
        }
        ...
        fn bench_100_u32_into_ascii(b: &mut Bencher) {
            b.iter(|| for item in INT_U32.iter() {
                item.itoa();
            })
        }
